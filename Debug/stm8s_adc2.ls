...
1854                     ; 330     temph = ADC2->DRH;
1856  04ec c65404        	ld	a,21508
1857  04ef 5f            	clrw	x
1858  04f0 97            	ld	xl,a
1859  04f1 1f04          	ldw	(OFST-1,sp),x
1861                     ; 332     temph = (uint16_t)(templ | (uint16_t)(temph << (uint8_t)8));
1863  04f3 1e04          	ldw	x,(OFST-1,sp)
1864  04f5 7b03          	ld	a,(OFST-2,sp)
1865  04f7 02            	rlwa	x,a
1866  04f8 1f04          	ldw	(OFST-1,sp),x
1869  04fa 2021          	jra	L735
1870  04fc               L535:
1871                     ; 337     temph = ADC2->DRH;
1873  04fc c65404        	ld	a,21508
1874  04ff 5f            	clrw	x
1875  0500 97            	ld	xl,a
1876  0501 1f04          	ldw	(OFST-1,sp),x
1878                     ; 339     templ = ADC2->DRL;
1880  0503 c65405        	ld	a,21509
1881  0506 6b03          	ld	(OFST-2,sp),a
1883                     ; 341     temph = (uint16_t)((uint16_t)((uint16_t)templ << 6) | (uint16_t)((uint16_t)temph << 8));
1885  0508 1e04          	ldw	x,(OFST-1,sp)
1886  050a 4f            	clr	a
1887  050b 02            	rlwa	x,a
1888  050c 1f01          	ldw	(OFST-4,sp),x
1890  050e 7b03          	ld	a,(OFST-2,sp)
1891  0510 97            	ld	xl,a
1892  0511 a640          	ld	a,#64
1893  0513 42            	mul	x,a
1894  0514 01            	rrwa	x,a
1895  0515 1a02          	or	a,(OFST-3,sp)
1896  0517 01            	rrwa	x,a
1897  0518 1a01          	or	a,(OFST-4,sp)
1898  051a 01            	rrwa	x,a
1899  051b 1f04          	ldw	(OFST-1,sp),x
1901  051d               L735:
1902                     ; 344   return ((uint16_t)temph);
1904  051d 1e04          	ldw	x,(OFST-1,sp)
1907  051f 5b05          	addw	sp,#5
1908  0521 81            	ret
1952                     ; 352 FlagStatus ADC2_GetFlagStatus(void)
1952                     ; 353 {
1953                     	switch	.text
1954  0522               _ADC2_GetFlagStatus:
1958                     ; 355   return (FlagStatus)(ADC2->CSR & ADC2_CSR_EOC);
1960  0522 c65400        	ld	a,21504
1961  0525 a480          	and	a,#128
1964  0527 81            	ret
1987                     ; 363 void ADC2_ClearFlag(void)
1987                     ; 364 {
1988                     	switch	.text
1989  0528               _ADC2_ClearFlag:
1993                     ; 365   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
1995  0528 721f5400      	bres	21504,#7
1996                     ; 366 }
1999  052c 81            	ret
2023                     ; 374 ITStatus ADC2_GetITStatus(void)
2023                     ; 375 {
2024                     	switch	.text
2025  052d               _ADC2_GetITStatus:
2029                     ; 376   return (ITStatus)(ADC2->CSR & ADC2_CSR_EOC);
2031  052d c65400        	ld	a,21504
2032  0530 a480          	and	a,#128
2035  0532 81            	ret
2059                     ; 384 void ADC2_ClearITPendingBit(void)
2059                     ; 385 {
2060                     	switch	.text
2061  0533               _ADC2_ClearITPendingBit:
2065                     ; 386   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
2067  0533 721f5400      	bres	21504,#7
2068                     ; 387 }
2071  0537 81            	ret
2084                     	xdef	_ADC2_ClearITPendingBit
2085                     	xdef	_ADC2_GetITStatus
2086                     	xdef	_ADC2_ClearFlag
2087                     	xdef	_ADC2_GetFlagStatus
2088                     	xdef	_ADC2_GetConversionValue
2089                     	xdef	_ADC2_StartConversion
2090                     	xdef	_ADC2_ExternalTriggerConfig
2091                     	xdef	_ADC2_ConversionConfig
2092                     	xdef	_ADC2_SchmittTriggerConfig
2093                     	xdef	_ADC2_PrescalerConfig
2094                     	xdef	_ADC2_ITConfig
2095                     	xdef	_ADC2_Cmd
2096                     	xdef	_ADC2_Init
2097                     	xdef	_ADC2_DeInit
2098                     	xref	_assert_failed
2099                     .const:	section	.text
2100  0000               L172:
2101  0000 7372635c7374  	dc.b	"src\stm8s_adc2.c",0
2121                     	end
